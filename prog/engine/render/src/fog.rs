...
pub struct FogCb {
    ...
    pub cascades: [Mat4; 4],
    /// Bindless index of the cloud-shadow map. 0 = skip (slot 0 is the dummy).
    pub cloud_shadow: u32,
    /// Full width of the map, in the same world units as [`Self::cloud_origin`].
    pub cloud_extent: f32,
    /// XZ centre of the map.
    pub cloud_origin: Vec2,
}
...
mod tests {
    ...
    fn cb_layout_matches_the_spvasm() {
        ...
        assert_eq!(std::mem::size_of::<FogCb>(), 480);
        ...
        assert_eq!(std::mem::offset_of!(FogCb, cloud_shadow), 464);
        assert_eq!(std::mem::offset_of!(FogCb, cloud_extent), 468);
        assert_eq!(std::mem::offset_of!(FogCb, cloud_origin), 472);
        ...
        let expected = [
            ...
            (19, offset_of!(FogCb, cloud_shadow) as u32),
            (20, offset_of!(FogCb, cloud_extent) as u32),
            (21, offset_of!(FogCb, cloud_origin) as u32),
        ];
        for shader in [
        ...
        crate::spvasm_layout::assert_glsl_offsets(
            "prog/samples/gates/fog/shaders/inject.cs.glsl",
            &expected,
        );
    }
